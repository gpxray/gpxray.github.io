...
function showHeroPreview() {
    ...
    if (heroClimbLoad) {
        const officialElevation = currentRaceConfig?.elevation;
        const elevation = officialElevation || gpxData.elevationGain;
        heroClimbLoad.textContent = `+${Math.round(elevation)}m`;
    }
...
function displayStats() {
    ...
    const officialElevation = currentRaceConfig?.elevation;
    document.getElementById('elevationGain').textContent = `${(officialElevation || gpxData.elevationGain).toFixed(0)} m`;
...
function printRaceCard() {
    ...
    const printElevGain = currentRaceConfig?.elevation || gpxData.elevationGain;
    document.getElementById('printElevation').textContent = `↑${printElevGain.toFixed(0)}m ↓${gpxData.elevationLoss.toFixed(0)}m`;
...
function generateMiniElevationProfile(showAid = true, showClimbs = true, unitLabel = 'km') {
    ...
    // Sample points for smoother SVG (every ~500m)
    ...
    const yScale = (elev) => margin.top + chartHeight - ((elev - minElev) / elevRange) * chartHeight;
    
    // Build path for the area fill
    let pathD = `M ${xScale(0)} ${margin.top + chartHeight}`;
    sampledPoints.forEach(p => {
        const x = xScale(p.distance);
        const y = yScale(p.elevation || minElev);
        pathD += ` L ${x} ${y}`;
    });
    ...
    // Add total D+ info - prefer official elevation when available
    const totalGain = Math.round(currentRaceConfig?.elevation || gpxData.elevationGain);
    ...
    return `
        <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="${pathD}" fill="url(#elevGradient)" stroke="#00d4ff" stroke-width="1.5"/>
            ${markersHtml}
            ${startLabel}
            ${finishLabel}
            ${legendGroup}
...
async function exportShareCardWithOptions(options = {}) {
    ...
    try {
        ...
        const officialElevation = currentRaceConfig?.elevation;
        const displayElevGain = officialElevation || gpxData.elevationGain;
        const statsSection = showStats ? `
            <div style="background: rgba(0,212,255,0.15); border-radius: 10px; padding: 12px 15px; margin-bottom: 15px;">
                ...
                <div style="display: flex; justify-content: center; gap: 20px; font-size: 14px; color: #ccc;">
                    ...
                    <span>+${displayElevGain.toFixed(0)}m</span>
    ...
    try {
        ...
        card.innerHTML = `
            ...
            <div style="text-align: center;">
                ...
                <div style="font-size: 26px; font-weight: 500; color: #ddd; margin-bottom: 20px;">${distance.toFixed(0)}${unitLabel} | ${(currentRaceConfig?.elevation || gpxData.elevationGain).toFixed(0)}m</div>
    ...
    try {
        ...
        if (includeAidStations && aidStations && aidStations.length > 0) {
            aidStations.forEach((aid, index) => {
                ...
                const shortName = aid.name ? aid.name.substring(0, 10) : `KM${Math.round(aid.km)}`;
                const aidNum = index + 1;
                waypoints.push({
                    lat: point.lat,
                    lon: point.lon,
        ...
        if (topClimbs && topClimbs.length > 0) {
            topClimbs.forEach((climb, index) => {
                ...
                waypoints.push({
                    lat: point.lat,
                    lon: point.lon,
                    name: `CLIMB${climbNum} +${Math.round(climb.gain)}m`,
